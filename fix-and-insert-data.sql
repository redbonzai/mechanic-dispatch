-- Fix existing data and insert missing data
-- This script fixes NULL updatedAt values and inserts missing data

-- Fix existing Mechanic records with NULL updatedAt
UPDATE "Mechanic" 
SET "updatedAt" = COALESCE("updatedAt", "createdAt", CURRENT_TIMESTAMP)
WHERE "updatedAt" IS NULL;

-- Fix existing Review records with NULL updatedAt
UPDATE "Review" 
SET "updatedAt" = COALESCE("updatedAt", "createdAt", CURRENT_TIMESTAMP)
WHERE "updatedAt" IS NULL;

-- Insert Mechanics if they don't exist (with updatedAt included)
...
) AS v("id", "name", "slug", "bio", "location", "yearsExperience", "rating", "reviewCount", "jobsCompleted", "sinceYear", "certifications", "badges", "isActive", "createdAt", "updatedAt")
...
-- Insert MechanicSkills (linking mechanics to their skills)
...
-- Insert Reviews (with updatedAt included)
